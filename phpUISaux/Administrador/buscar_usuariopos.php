	...
	<!-- Register -->
					...
					<div class="search_section d-flex flex-column align-items-center justify-content-center">
						...
						<div class="search_content text-center">
                            ...
                            <h1 class="search_title">Listar Postulaciones</h1>
</div>
</div>
</div>
...
<?php 
		...
		<a href="registro_postulacion.php" class="btn_new">Crear Postulacion</a>
		
		<form action="buscar_usuariopos.php" method="get" class="form_search">
		...
		<?php 
			...
			$sql_registe = mysqli_query($conn,"SELECT COUNT(*) as total_postulaciones FROM postulaciones 
																WHERE ( id LIKE '%$busqueda%' OR 
																		aspirante LIKE '%$busqueda%' OR 
																		fecha_postulacion LIKE '%$busqueda%' OR 
																		fecha_dada_baja LIKE '%$busqueda%' OR
																		estado LIKE '%$busqueda%' OR
																		auxiliatura LIKE '%$busqueda%' 
																		$estado  )  ");

			$result_register = mysqli_fetch_array($sql_registe);
			$total_registro = $result_register['total_postulaciones'];
			...
			$query = mysqli_query($conn,"SELECT p.idpos, p.aspirante, p.fecha_postulacion,p.fecha_dada_baja, p.estado,p.auxiliatura FROM postulaciones p
										WHERE 
										( p.idpos LIKE '%$busqueda%' OR 
											p.aspirante LIKE '%$busqueda%' OR 
											p.fecha_postulacion LIKE '%$busqueda%' OR 
											p.fecha_dada_baja LIKE '%$busqueda%' OR
											p.estado LIKE '%$busqueda%' OR
											p.auxiliatura LIKE '%$busqueda%' ) 
										 ORDER BY p.idpos ASC LIMIT $desde,$por_pagina 
				");
			mysqli_close($conn);
			...
			?>
				<tr>
				<td><?php echo $data["idpos"]; ?></td>
					<td><?php echo $data["aspirante"]; ?></td>
					<td><?php echo $data["fecha_postulacion"]; ?></td>
					<td><?php echo $data["fecha_dada_baja"]; ?></td>
					<td><?php echo $data['estado'] ?></td>
					<td><?php echo $data['auxiliatura'] ?></td>
					<td>
						<a class="link_edit" href="editar_usuariopos.php?id=<?php echo $data["idpos"]; ?>">Editar</a>

					<?php if($data["idusuario"] != 1){ ?>
						|
						<a class="link_delete" href="eliminar_confirmar_usuariopos.php?id=<?php echo $data["idpos"]; ?>">Eliminar</a>
					<?php } ?>
