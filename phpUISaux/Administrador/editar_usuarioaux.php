...
if(!empty($_POST))
	{
		$alert='';
		if(empty($_POST['fecha_inicio']) || empty($_POST['estado']) || empty($_POST['texto']))
		{
			...
			$query = mysqli_query($conn,"SELECT * FROM auxiliaturas 
													   WHERE (texto = '$texto' AND idaux != $idaux)
													   OR (estado = '$estado' AND idaux != $idaux) ");
			...
			if($result > 0){
				...
				if(empty($_POST['fecha_finalizacion']))
				{
					...
					$sql_update = mysqli_query($conn,"UPDATE auxiliaturas
															SET fecha_inicio = '$fecha_inicio', estado='$estado',texto='$texto'
															WHERE idaux= $idaux ");
				}else{
					$sql_update = mysqli_query($conn,"UPDATE auxiliaturas
															SET fecha_inicio = '$fecha_inicio', estado='$estado',texto='$texto',fecha_finalizacion='$fecha_finalizacion'
															WHERE idaux= $idaux ");

				}

				if($sql_update){
					$alert='<p class="msg_save">Auxiliatura actualizado correctamente.</p>';
				}else{
					$alert='<p class="msg_error">Error al actualizar la auxiliatura.</p>';
				}
	...
	$sql= mysqli_query($conn,"SELECT a.idaux,a.fecha_inicio, a.Administrativo_creador, a.estado, a.texto,a.auxiliar, a.fecha_finalizacion FROM auxiliaturas a
	WHERE a.idaux = $idaux ");
...
<html lang="es">
<head>
<title>Editar-auxiliatura</title>
<meta charset="utf-8">
...
<h1 class="search_title">Actualizar Auxiliatura</h1>
</div>
</div>
</div>
			...
			<form action="" method="post" class="search_form">
				...
				<input type="date" name="fecha_inicio" id="fecha_inicio" class="input_field search_form_category" placeholder="Fecha inicio" value="<?php echo $fecha_inicio; ?>">
				...
				<input type="text" name="texto" id="texto" class="input_field search_form_category" placeholder="texto" value="<?php echo $texto; ?>">
				<label for="fecha_finalizacion">Fecha finalización:</label>
				<input type="date" name="fecha_finalizacion" id="fecha_finalizacion"  class="input_field search_form_category" placeholder="Fecha finalizacion">
		<br><br>
		<span><?php echo isset($alert) ? $alert : ''; ?></p>
				<a  id="search_submit_button" href="Administraraux.php" class="btn_cancel">Atras</a>
				<input id="search_submit_button" type="submit" value="Actualizar auxiliatura" class="btn_ok">
