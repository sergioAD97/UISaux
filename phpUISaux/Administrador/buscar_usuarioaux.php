	...
	<!-- Register -->
					...
					<div class="search_section d-flex flex-column align-items-center justify-content-center">
						...
						<div class="search_content text-center">
                            ...
                            <h1 class="search_title">Lista de Auxiliaturas</h1>
</div>
</div>
</div>
...
<?php 
		...
		<a href="registro_auxiliatura.php" class="btn_new">Crear Auxiliatura</a>
		<form action="buscar_usuarioaux.php" method="get" class="form_search">
		...
		<?php 
			...
			if($busqueda == 'A')
			{
				$estado = " OR rol LIKE '%1%' ";

			}else if($busqueda == 'P'){

				$estado = " OR rol LIKE '%2%' ";

			}else if($busqueda == 'E'){

				$estado = " OR rol LIKE '%3%' ";
			}


			$sql_registe = mysqli_query($conn,"SELECT COUNT(*) as total_auxiliatura FROM auxiliaturas 
																WHERE ( id LIKE '%$busqueda%' OR 
																		aspirante LIKE '%$busqueda%' OR 
																		fecha_inicio LIKE '%$busqueda%' OR 
																		Administrativo_creador LIKE '%$busqueda%' OR
																		estado LIKE '%$busqueda%' OR
																		texto LIKE '%$busqueda%' OR
																		auxiliar LIKE '%$busqueda%' OR
																		fecha_finalizacion LIKE '%$busqueda%' 
																		$estado  )  ");

			$result_register = mysqli_fetch_array($sql_registe);
			$total_registro = $result_register['total_auxiliatura'];
			...
			$query = mysqli_query($conn,"SELECT a.idaux, a.fecha_inicio, a.Administrativo_creador, a.estado, a.texto,a.auxiliar, a.fecha_finalizacion FROM auxiliaturas a
										WHERE 
										( a.idaux LIKE '%$busqueda%' OR 
										a.fecha_inicio LIKE '%$busqueda%' OR 
										a.Administrativo_creador LIKE '%$busqueda%' OR 
										a.estado LIKE '%$busqueda%' OR 
										a.texto LIKE '%$busqueda%' OR 
										a.auxiliar LIKE '%$busqueda%' OR 
										a.fecha_finalizacion LIKE '%$busqueda%' ) 
										 ORDER BY a.idaux ASC LIMIT $desde,$por_pagina 
				");
			mysqli_close($conn);
			...
			?>
				<tr>
				<td><?php echo $data["idaux"]; ?></td>
					<td><?php echo $data["fecha_inicio"]; ?></td>
					<td><?php echo $data["Administrativo_creador"]; ?></td>
					<td><?php echo $data["estado"]; ?></td>
					<td><?php echo $data['texto'] ?></td>
					<td><?php echo $data['auxiliar'] ?></td>
					<td><?php echo $data['fecha_finalizacion'] ?></td>
					<td>
						<a class="link_edit" href="editar_usuarioaux.php?id=<?php echo $data["idaux"]; ?>">Editar</a>

					<?php if($data["idusuario"] != 1){ ?>
						|
						<a class="link_delete" href="eliminar_confirmar_usuarioaux.php?id=<?php echo $data["idaux"]; ?>">Eliminar</a>
					<?php } ?>
