...
{
	$alert='';
	if(empty($_POST['aspirante']) || empty($_POST['fecha_postulacion']) || empty($_POST['fecha_dada_baja']) || empty($_POST['estado']) || empty($_POST['auxiliatura']))
	{
		...
		$query = mysqli_query($conn,"SELECT * FROM postulaciones WHERE  idpos = '$idpos' ");
		$result = mysqli_fetch_array($query);

		if($result > 0){
			$alert='<p class="msg_error">La postulación ya existe.</p>';
		}else{

			$query_insert = mysqli_query($conn,"INSERT INTO postulaciones(aspirante,fecha_postulacion,fecha_dada_baja,estado,auxiliatura)
																VALUES('$aspirante','$fecha_postulacion','$fecha_dada_baja','$estado','$auxiliatura')");
			if($query_insert){
				$alert='<p class="msg_save">Postulación creado correctamente.</p>';
			}else{
				$alert='<p class="msg_error">Error al crear la postulacion.</p>';
			}
	...
	<!-- Register -->
					...
					<div class="search_section d-flex flex-column align-items-center justify-content-center">
						...
						<div class="search_content text-center">
                            ...
                            <h1 class="search_title">Registro Postulación</h1>
</div>
</div>
</div>
...
<div class="register">
			...
			<form action="" method="post"  class="search_form">
				...
				<input type="text" name="aspirante" id="aspirante" placeholder="Aspirante" class="input_field search_form_category">
				<label for="fecha_postulacion">Fecha Postulación</label>
				<input type="date" name="fecha_postulacion" id="fecha_postulacion" placeholder="Fecha postulacion" class="input_field search_form_category">
				<label for="fecha_dada_baja">Fecha dada baja</label>
				<input type="date" name="fecha_dada_baja" id="fecha_dada_baja" placeholder="Fecha dada baja" class="input_field search_form_category">
				<label for="estado">estado</label>
				<input type="text" name="estado" id="estado" placeholder="Estado" class="input_field search_form_category">
				<label for="auxiliatura">Auxiliatura</label>
				<input type="text" name="auxiliatura" id="auxiliatura" placeholder="Auxiliatura" class="input_field search_form_category">
